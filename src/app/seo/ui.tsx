...
/** Short plain-language "what to do with this view" line under a tab title. */
export function Explainer({ title, children }: { title: string; children: ReactNode }) {
  return (
    <Box sx={{ mb: 2.5 }}>
      <Typography
        sx={{ fontFamily: DISPLAY, fontSize: "1.15rem", fontWeight: 600, color: INK, letterSpacing: "-0.02em" }}
      >
        {title}
      </Typography>
      <Typography sx={{ mt: 0.5, fontSize: "0.875rem", color: MUTED, lineHeight: 1.6, maxWidth: 860 }}>
        {children}
      </Typography>
    </Box>
  );
...
export function EmptyState({ title, body }: { title: string; body: string }) {
  return (
    <Box sx={{ px: 3, py: 6, textAlign: "center" }}>
      ...
      <Typography sx={{ mt: 0.5, fontSize: "0.84rem", color: MUTED, maxWidth: 520, mx: "auto", lineHeight: 1.6 }}>
        {body}
      </Typography>
    </Box>
  );
