...
type Brain = {
  brandVoice?: { audiences?: string[]; signaturePhrases?: string[] };
  personas?: Persona[];
...
export default function CreateStudio() {
  ...
  const filters = useMemo(
    ...
    [language, audience, category, primaryKeyword, secondaryKeywords, creativity, length, withImage]
  );
  ...
  const enhanceBrief = async () => {
    ...
    try {
      const res = await fetch("/api/brief", {
        ...
        body: JSON.stringify({ channel, topic: topic.trim(), personaId: personaId || undefined, filters }),
      });
    ...
    try {
      if (mode === "draft") {
        ...
        const res = await fetch("/api/propose", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ channel, topic: topic.trim(), filters, personaId: personaId || undefined }),
        });
        const data = await res.json();
        if (!res.ok) setError(data.error ?? "Generation failed");
        else
              ...
              <Box sx={{ display: "flex", flexDirection: "column", gap: 0.75, mb: 2 }}>
                <Box
                  ...
                  sx={{
                    p: 1.25,
                    borderRadius: 1.5,
                    border: `1.5px solid ${personaId === "" ? NAVY : "#e6e8ec"}`,
                    bgcolor: personaId === "" ? "#e8f0f4" : "#fff",
                    cursor: "pointer",
                  }}
                >
                  <Typography sx={{ fontSize: 13, fontWeight: 600, color: "#1a1d21" }}>General audience</Typography>
                  <Typography sx={{ fontSize: 11.5, color: "#5b6470" }}>Brand voice without a specific persona</Typography>
                </Box>
                {personas.map((p) => (
                  <Box
                    key={p.id}
                    onClick={() => setPersonaId(personaId === p.id ? "" : p.id)}
                    sx={{
                      p: 1.25,
                      borderRadius: 1.5,
                      border: `1.5px solid ${personaId === p.id ? NAVY : "#e6e8ec"}`,
                      bgcolor: personaId === p.id ? "#e8f0f4" : "#fff",
                      cursor: "pointer",
                  ...
                  >
                    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                      <Box sx={{ width: 26, height: 26, borderRadius: "50%", bgcolor: personaId === p.id ? NAVY : "#f0f1f3", color: personaId === p.id ? "#fff" : "#5b6470", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10.5, fontWeight: 800 }}>
                        {p.code}
                      </Box>
                      <Box sx={{ minWidth: 0 }}>
              ...
              {activePersona ? (
                <Box sx={{ p: 1.5, borderRadius: 1.5, bgcolor: "#e8f0f4", mb: 1.5 }}>
                  <Typography sx={{ fontSize: 12.5, fontWeight: 700, color: NAVY }}>
                    Writing for {activePersona.code} — {activePersona.name}
                  </Typography>
                  <Typography sx={{ fontSize: 11.5, color: "#3c4043", mt: 0.5, lineHeight: 1.5 }}>
